...
void MakeAndSendOutputCommand(char* deviceName, char* deviceCmd, int err, char* values[], int nvalues) {
  ...
  Serial.print(ummhwords::sepIn);
  Serial.print(deviceCmd);
  Serial.print(ummhwords::sepIn);
  Serial.print(err);
  if (nvalues>0) {
    Serial.print(ummhwords::sepWithin);
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (strcmp(ummhwords::device_list_start,cmdIn)==0) { // take care of special commands first
      dd = 0;
      Serial.print(pDeviceDescriptions[dd]);
      Serial.print(ummhwords::sepEnd);
      dd++;
    } else if (strcmp(ummhwords::device_list_continue,cmdIn)==0) {
      Serial.print(pDeviceDescriptions[dd]);
      Serial.print(ummhwords::sepEnd);
      if (strcmp(pDeviceDescriptions[dd],ummhwords::device_list_end)==0) {
        dd=0;
      } else {
        dd++;
      }
      ...
      GetDeviceFromCmdIn(ummhwords::sepOut);
      GetCommandFromCmdIn(ummhwords::sepOut);
      
      if (strcmp(strDevice,"Shutter-A")==0) {
        GetNextValue(ummhwords::sepWithin);
        if (strcmp(strCommand,"SO")==0) {
          ...
          MakeAndSendOutputCommand(strDevice, ummhwords::timeout, 1, new char*[1] {"1600"}, 1);
          delay(1500);
          MakeAndSendOutputCommand(strDevice, strCommand, 0, new char*[1] {strSingleValue}, 1);
          MakeAndSendOutputCommand(strDevice, ummhwords::timeout, 0, new char*[1] {strDefaultTimeout}, 1);
          ...
          MakeAndSendOutputCommand(strDevice, strCommand, ummherrors::ctr_device_command_not_recognized, new char*[0], 0);          
        }
        ...
        MakeAndSendOutputCommand(strDevice, strCommand, ummherrors::ctr_device_not_recognized, new char*[0], 0);
      }
